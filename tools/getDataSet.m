...
%                data = data.raw;
    ...
    %% Search for dataset
    app.LogTextArea.Value = [app.LogTextArea.Value; {char("[getDataSet.m] Searching for DAQ file in " + exp + "...")}];
    
    if isunix 
        %% assumes running from facet-srv20
        if nargin == 2
        ...
        %Search sequentially back in time
        currentYear = year(datetime('now'));
        [~, outp] = unix(sprintf( 'find %s -name "*%s" 2>/dev/null', [searchPath num2str(currentYear) '/'], dataSetID ));
        disp([searchPath num2str(currentYear) '/'])
        disp(dataSetID)
        if isempty(outp)
    ...
    if isempty(outp)
        ...
        app.LogTextArea.Value = [app.LogTextArea.Value; {'[getDataSet.m] Found dataset!'}];
    end
    ...
    if exist(matfile)
        load(matfile);
        app.LogTextArea.Value = [app.LogTextArea.Value; {char("[getDataSet.m] .mat file found: " + matfile)}];
    else
    ...
    app.LogTextArea.Value = [app.LogTextArea.Value; {char("[getDataSet.m] header: " + header)}];
